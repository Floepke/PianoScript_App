...
class DrawEditor:
    ...
    def draw_titles(io: dict):
        ...
        title = "'" + io['score']['header']['title'] + "'" + \
            ' by composer: ' + io['score']['header']['composer']
    ...
    def draw_barlines_grid_timesignature_and_measurenumbers(io: dict, top, bttm):
        ...
        io['editor'].delete_with_tag(
            ['barline', 'timesignature', 'measurenumber', 'gridline'])
    ...
    def add_soundingdots_and_stopsigns_to_viewport(io):
        ...
        for note in io['viewport']['events']['note']:
            ...
            note_on_off.append(copy.deepcopy(evt))
        note_on_off = sorted(
            note_on_off, key=lambda y: y['time'] if y['type'] == 'note' else y['endtime'])
        ...
        for idx, note in enumerate(note_on_off):
            ...
            if stop_flag:
                stop_sign(note['time']+note['duration'] -
                          FRACTION, note['pitch'], note)
